...
NodeID3.prototype.update = function (tags, filebuffer, fn) {
  ...
  if (!fn || typeof fn !== "function") {
    ...
    Object.keys(rawTags).map(function (tag) {
      if (
        SFrames[SRawToNameMap[tag]] &&
        SFrames[SRawToNameMap[tag]].multiple &&
        currentTags[tag] &&
        rawTags[tag]
      ) {
        cCompare = {};
        ...
        rawTags[tag].forEach((rTag, index) => {
          let comparison =
            cCompare[rTag[SFrames[SRawToNameMap[tag]].updateCompareKey]];
      ...
      function (err, currentTags) {
        ...
        Object.keys(rawTags).map(function (tag) {
          if (
            ...
            currentTags[tag].forEach((cTag, index) => {
              cCompare[
                cTag[SFrames[SRawToNameMap[tag]].updateCompareKey]
              ] = index;
            });
            if (!(rawTags[tag] instanceof Array)) rawTags[tag] = [rawTags[tag]];
            rawTags[tag].forEach((rTag, index) => {
...
NodeID3.prototype.getTagsFromBuffer = function (filebuffer, options) {
  ...
  let frameSize =
    this.getTagSize(
      Buffer.from(
        filebuffer.toString("hex", framePosition, framePosition + 10),
        "hex"
      )
    ) + 10;
  ...
  let frames = this.getFramesFromID3Body(
    ID3FrameBody,
    ID3Version,
    identifierSize,
    textframeHeaderSize
...
NodeID3.prototype.getFramesFromID3Body = function (
  ID3FrameBody,
  ID3Version,
  identifierSize,
  textframeHeaderSize
  ...
  let frames = [];
  while (
    currentPosition < ID3FrameBody.length &&
    ID3FrameBody[currentPosition] !== 0x00
  ) {
    ...
    let bodyFrameSize = this.getFrameSize(
      bodyFrameHeader,
      decodeSize,
      ID3Version
    );
    ...
    ID3FrameBody.copy(
      bodyFrameBuffer,
      0,
      currentPosition + textframeHeaderSize
    );
    ...
    function (frame, index) {
      ...
      if (frame.name[0] === "T" && frame.name !== "TXXX") {
        ...
        if (frame.body[0] === 0x01) {
          decoded = iconv
            .decode(frame.body.slice(1), "utf16")
            .replace(/\0/g, "");
        } else {
          decoded = iconv
            .decode(frame.body.slice(1), "ISO-8859-1")
            .replace(/\0/g, "");
        }
        ...
        Object.keys(versionFrames).map(
          function (key) {
            if (versionFrames[key].name === frame.name) {
              let decoded = this[versionFrames[key].read](
                frame.body,
                ID3Version
              );
...
NodeID3.prototype.getTagSize = function (buffer) {
  return this.decodeSize(
    Buffer.from([buffer[6], buffer[7], buffer[8], buffer[9]])
  );
};
...
NodeID3.prototype.createPictureFrame = function (data) {
  try {
    if (
      data &&
      data.imageBuffer &&
      data.imageBuffer instanceof Buffer === true
    ) {
...
NodeID3.prototype.readPictureFrame = function (APICFrame, ID3Version) {
  ...
  if (ID3Version == 2) {
    ...
    APICMimeType = APICFrame.toString("ascii").substring(
      1,
      APICFrame.indexOf(0x00, 1)
    );
  }
  ...
  if (APICFrame[0] == 0x00) {
    if (ID3Version == 2) {
      picture.description =
        iconv.decode(
          APICFrame.slice(5, APICFrame.indexOf(0x00, 5)),
          "ISO-8859-1"
        ) || undefined;
      ...
      picture.description =
        iconv.decode(
          APICFrame.slice(
            APICFrame.indexOf(0x00, 1) + 2,
            APICFrame.indexOf(0x00, APICFrame.indexOf(0x00, 1) + 2)
          ),
          "ISO-8859-1"
        ) || undefined;
    ...
    if (ID3Version == 2) {
      ...
      if (descFound != -1) {
        picture.description =
          iconv.decode(desc.slice(0, descFound + 2), "utf16") || undefined;
      }
    } else {
...
NodeID3.prototype.createContentDescriptor = function (
  description,
  encoding,
  terminated
) {
  if (!description) {
    description = terminated
      ? iconv.encode("\0", this.getEncodingName(encoding))
      : Buffer.alloc(0);
    return description;
  }
  ...
  return terminated
    ? Buffer.concat([
        description,
        Buffer.alloc(this.getTerminationCount(encoding)).fill(0x00),
      ])
    : description;
};
...
NodeID3.prototype.createText = function (text, encoding, terminated) {
  ...
  return terminated
    ? Buffer.concat([
        text,
        Buffer.from(this.getTerminationCount(encoding)).fill(0x00),
      ])
    : text;
};
...
NodeID3.prototype.createCommentFrame = function (comment) {
  ...
  let descriptorBuffer = this.createContentDescriptor(
    comment.shortText,
    0x01,
    true
  );
...
NodeID3.prototype.readCommentFrame = function (frame) {
  ...
  if (frame[0] == 0x00) {
    ...
    while (
      (frame[descriptorEscape] !== undefined &&
        frame[descriptorEscape] !== 0x00) ||
 ...
 **  }
 **/
NodeID3.prototype.createUnsynchronisedLyricsFrame = function (
  unsynchronisedLyrics
) {
  unsynchronisedLyrics = unsynchronisedLyrics || {};
  if (
    typeof unsynchronisedLyrics === "string" ||
    unsynchronisedLyrics instanceof String
  ) {
    unsynchronisedLyrics = {
  ...
  let descriptorBuffer = this.createContentDescriptor(
    unsynchronisedLyrics.shortText,
    0x01,
    true
  );
 ...
 **  }
 **/
NodeID3.prototype.createUserDefinedText = function (
  userDefinedText,
  recursiveBuffer
) {
  udt = userDefinedText || {};
  ...
  if (udt && udt.description) {
    ...
    buffer.writeUInt32BE(
      encodingBuffer.length + descriptorBuffer.length + valueBuffer.length,
      4
    );
    if (!recursiveBuffer) {
      recursiveBuffer = Buffer.concat([
        buffer,
        encodingBuffer,
        descriptorBuffer,
        valueBuffer,
...
NodeID3.prototype.createPopularimeterFrame = function (popularimeter) {
  ...
  buffer.writeUInt32BE(
    emailBuffer.length + ratingBuffer.length + counterBuffer.length,
    4
  );
...
NodeID3.prototype.readPrivateFrame = function (frame) {
  ...
  tags.ownerIdentifier = iconv.decode(
    frame.slice(0, endOfOwnerIdentification),
    "ISO-8859-1"
  );
...
NodeID3.prototype.readChapterFrame = function (frame) {
  ...
  if (
    endOfElementIDString == -1 ||
    frame.length - endOfElementIDString - 1 < 16
  ) {
    return tags;
  }

  tags.elementID = iconv.decode(
    frame.slice(0, endOfElementIDString),
    "ISO-8859-1"
  );
  tags.startTimeMs = frame.readUInt32BE(endOfElementIDString + 1);
  tags.endTimeMs = frame.readUInt32BE(endOfElementIDString + 5);
  if (
    frame.readUInt32BE(endOfElementIDString + 9) !=
    Buffer.alloc(4, 0xff).readUInt32BE()
  ) {
    tags.startOffsetBytes = frame.readUInt32BE(endOfElementIDString + 9);
  }
  if (
    frame.readUInt32BE(endOfElementIDString + 13) !=
    Buffer.alloc(4, 0xff).readUInt32BE()
  ) {
  ...
  if (frame.length - endOfElementIDString - 17 > 0) {
    ...
    tags.tags = this.getTagsFromFrames(
      this.getFramesFromID3Body(framesBuffer, 3, 4, 10),
      3
    );
  }
